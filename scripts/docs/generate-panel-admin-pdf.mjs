/**
 * Genera docs/PANEL-ADMINISTRACION.pdf desde docs/PANEL-ADMINISTRACION.md
 * Uso: node scripts/docs/generate-panel-admin-pdf.mjs
 */
...
const mdPath = join(root, "docs", "cliente", "PANEL-ADMINISTRACION.md");
const pdfPath = join(root, "docs", "cliente", "PANEL-ADMINISTRACION.pdf");
...
function generatePdf(blocks) {
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text(
      sanitizePdfText(
        `GEO ARTE CDMX - Panel de Administracion  |  Pagina ${p} de ${totalPages}`,
      ),
      margin,
      pageHeight - 8,
    );
