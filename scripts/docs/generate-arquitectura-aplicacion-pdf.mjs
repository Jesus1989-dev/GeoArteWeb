/**
 * Genera docs/ARQUITECTURA-APLICACION.pdf desde docs/ARQUITECTURA-APLICACION.md
 * Uso: node scripts/docs/generate-arquitectura-aplicacion-pdf.mjs
 */
...
const mdPath = join(root, "docs", "tecnico", "ARQUITECTURA-APLICACION.md");
const pdfPath = join(root, "docs", "tecnico", "ARQUITECTURA-APLICACION.pdf");
...
function parseMarkdown(md) {
  ...
  while (i < lines.length) {
    ...
    if (/^>\s/.test(line)) {
      ...
      blocks.push({ type: "paragraph", text: quoteLines.join(" ") });
      continue;
    }
...
function generatePdf(blocks) {
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text(
      sanitizePdfText(
        `GEO ARTE CDMX - Arquitectura de la aplicacion  |  Pagina ${p} de ${totalPages}`,
      ),
      margin,
      pageHeight - 8,
    );
